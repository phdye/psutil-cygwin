#!/usr/bin/env python3
"""
Direct fix for issue 014 - eliminate any orphaned finally blocks.

This script will create a clean version of the test file without any syntax errors.
"""

def create_clean_test_file():
    """Create a clean version of the test file without orphaned finally blocks."""
...
class TestRealTransparentImport(unittest.TestCase):
    ...
    def test_transparent_import_basic_functionality(self):
        ...
        try:
            ...
            # If psutil import fails completely, it means no psutil.pth file exists
            # and psutil_cygwin isn't available as 'psutil'
        ...
        if hasattr(psutil, '__name__') and psutil.__name__ == 'psutil_cygwin':
            ...
            # Standard psutil is installed instead
            print(f"⚠️  Standard psutil detected: {psutil.__name__}")
            print("   Transparent import is not active")
            print("   To activate: psutil-cygwin-setup install")
            self.skipTest(f"Standard psutil detected ({psutil.__name__}), not psutil-cygwin")
        else:
            # Unknown psutil module
...
'''
    ...
    output_file = "tests/test_pth_functionality.py"
    with open(output_file, 'w') as f:
        f.write(content)
    
    print(f"✅ Created clean test file: {output_file}")
